...
export default function Teams() {
  ...
  const { translateRole, translateDB } = useDBTranslation();
  ...
  const liveData = useCallback(async () => {
    ...
    try {
      ...
      setLoadError(err?.message || t("teams.loading"));
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(t("teams.deleteSuccess"), { id: loadingToast });
      } else {
        throw new Error("Failed to delete team member");
      }
    } catch (err) {
      console.error("Failed to delete team member:", err);
      toast.error(t("general.error"), { id: loadingToast });
      if (targetRow) {
  ...
  const handleAddMember = async (name: string, email: string, role: "admin" | "manager" | "support_agent") => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(t("teams.inviteSuccess"), { id: loadingToast });
      } else {
  ...
  const handleUpdateMember = async (documentId: string, data: { memberEmail: string; statusAccess: "admin" | "manager" | "support_agent" }) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(t("teams.updateSuccess"), { id: loadingToast });
      } else {
  ...
  const getRoleLabel = (role: string) => translateRole(role);
  const getPermissionLabel = (role: string) => {
    if (role === "admin") return t("teams.admin") === "Admin" ? "Full Access" : "وصول كامل";
    if (role === "manager") return t("teams.manager") === "Manager" ? "Products & Orders" : "المنتجات والطلبات";
    return t("teams.support") === "Support Agent" ? "Tickets Only" : "التذاكر فقط";
  };
            ...
            <table className="w-full min-w-[700px] text-start border-collapse">
              <thead>
                <tr className="border-b border-slate-100 dark:border-zinc-800/80 bg-slate-50/50 dark:bg-zinc-900/50 text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">
                  ...
                  <th className="p-4">{t("teams.role")}</th>
                  <th className="p-4">{t("general.bilingual")}</th>
                  <th className="p-4">{t("teams.status")}</th>
                ...
                {teamData.map((member: TeamMember) => {
                  ...
                  return (
                    <tr key={member.documentId || member.id || member.memberEmail} data-id={member.documentId} className="hover:bg-slate-50/40 dark:hover:bg-zinc-800/20 transition-colors text-sm text-slate-700 dark:text-zinc-300">
                      ...
                      <td className="p-4 font-semibold text-slate-900 dark:text-white">{getRoleLabel(member.statusAccess)}</td>
                      <td className="p-4">
                        ...
                        `}>
                          ...
                          {getPermissionLabel(member.statusAccess)}
                        </span>
                      </td>
                      <td className="p-4">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold  ${member.statusMember === true ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400" : "text-red-500 bg-red-200 dark:bg-red-900/30 dark:text-red-500"}`}>
                          ...
                          {member.statusMember === true ? t("teams.active") : t("teams.inactive")}
                        </span>
                      </td>
                        ...
                        {isDropdownOpen && (
                          <>
                            ...
                            <div className={`absolute ${dir === "rtl" ? "left-4" : "right-4"} mt-1 w-36 bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-slate-100 dark:border-zinc-700/50 py-1.5 z-50 animate-scale-in text-start`}>
                              <button onClick={() => { setEditingMember(member); setActiveDropdownId(""); }} className="w-full text-start px-3.5 py-2 text-xs font-bold text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 transition-colors flex items-center gap-2 cursor-pointer">
                                <Pencil size={14} />
                                {t("teams.edit")}
                              </button>
                              <button onClick={(e) => { handleDelete((member as TeamMember).documentId ?? "", e); setActiveDropdownId(""); }} className="w-full text-start px-3.5 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/20 transition-colors flex items-center gap-2 cursor-pointer">
                                <Trash2 size={14} />
                                {t("teams.remove")}
                              </button>
                            </div>
