...
export default function OrdersTable() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setLoadError(err?.message || "Failed to fetch orders data.");
    } finally {
      setLoading(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-zinc-800/80 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-400 dark:text-zinc-500 mt-1">
            Track real-time order shipments, view delivery statuses, search customers, and inspect failed delivery logs.
          </p>
        </div>
      </div>
        ...
        <>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50 dark:bg-zinc-900/40 p-4 rounded-2xl border border-slate-100 dark:border-zinc-800/60">
            <div className="flex bg-slate-100/70 dark:bg-zinc-800/60 p-1.5 rounded-2xl border border-slate-200/40 dark:border-zinc-700/30 w-fit">
              {["All", "delivered", "inTransit", "failed"].map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  ...
                  {tab === "delivered" ? "Delivered" : tab === "inTransit" ? "In Transit" : tab === "failed" ? "Failed" : "All"}
                </button>
              ))}
            </div>
            <div className="relative flex-1 max-w-md">
              ...
              <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search by ID or customer name..."
                    ...
                    {filteredOrders.map((order) => (
                      <tr key={order.id} className="hover:bg-slate-50/40 dark:hover:bg-zinc-800/20 transition-colors text-sm text-slate-700 dark:text-zinc-300">
                        ...
                        <td className="p-4 font-medium text-slate-600 dark:text-zinc-400">
                          {order.orderItems && order.orderItems.length > 0 ? `${order.orderItems.reduce((sum: number, item: any) => sum + (item.quantity || 0), 0)} x Items` : '—'}
                        </td>
                        ...
                        <td className="p-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${
                            ...
                            {order.statusOrder === "delivered" ? "Delivered" : order.statusOrder === "inTransit" ? "In Transit" : "Failed"}
                          </span>
                        </td>
                        <td className="p-4">
          ...
          {selectedOrder && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl p-6 w-full max-w-md shadow-xl z-50 animate-scale-in text-left relative">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Inspection Order Logs</h3>
                <div className="space-y-4 text-xs font-medium">
                  <div className="bg-slate-50 dark:bg-zinc-800/40 p-3 rounded-xl border border-slate-100 dark:border-zinc-800">
                    <span className="text-slate-400 block font-bold uppercase tracking-wider">Log Status Message</span>
                    <p className={`mt-1 font-bold text-sm ${selectedOrder.statusOrder === "failed" ? "text-rose-500" : "text-slate-700 dark:text-zinc-300"}`}>
                      {selectedOrder.statusOrder === "failed" ? "Shipping Failed / Order Abandoned" : selectedOrder.statusOrder === "delivered" ? "Successful Distribution" : "In Transit"}
                    </p>
                  </div>
                  {selectedOrder.notes && (
                    <div className="bg-slate-50 dark:bg-zinc-800/40 p-3 rounded-xl border border-slate-100 dark:border-zinc-800">
                      <span className="text-slate-400 block font-bold uppercase tracking-wider">Notes</span>
                      <p className="mt-1 text-slate-700 dark:text-zinc-300 leading-relaxed font-semibold">{selectedOrder.notes}</p>
                    </div>
                  )}
                  {selectedOrder.customerAddress && (
                    <div className="bg-slate-50 dark:bg-zinc-800/40 p-3 rounded-xl border border-slate-100 dark:border-zinc-800">
                      <span className="text-slate-400 block font-bold uppercase tracking-wider">Address</span>
                      <p className="mt-1 text-slate-700 dark:text-zinc-300 leading-relaxed font-semibold">{selectedOrder.customerAddress}</p>
                    </div>
                  )}
                  {selectedOrder.customerPhone && (
                    <div className="bg-slate-50 dark:bg-zinc-800/40 p-3 rounded-xl border border-slate-100 dark:border-zinc-800">
                      <span className="text-slate-400 block font-bold uppercase tracking-wider">Phone</span>
